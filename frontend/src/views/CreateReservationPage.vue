...
function clearPickupLocationFeedback() {
  ...
  if (fieldErrors.value.dropoffLocation === SAME_ORIGIN_DEST_MSG) {
...
function clearDropoffLocationFeedback() {
  clearFieldError('dropoffLocation');
  if (fieldErrors.value.pickupLocation === SAME_ORIGIN_DEST_MSG) {
    clearFieldError('pickupLocation');
  }
}
...
function validateFormFields(): Partial<Record<FormFieldKey, string>> {
  ...
  if (form.riderFirstName.trim().length < 1) errors.riderFirstName = 'First name cant be empty';
  if (form.riderLastName.trim().length < 1) errors.riderLastName = 'Last name cant be empty';

  const pickupTrim = form.pickupLocation.trim();
  const dropTrim = form.dropoffLocation.trim();

  if (pickupTrim.length < 1) errors.pickupLocation = 'Pickup location cant be empty.';
  if (dropTrim.length < 1) errors.dropoffLocation = 'Dropoff location cant be empty.';
  if (
    pickupTrim.length > 0 &&
    dropTrim.length > 0 &&
  ...
  if (
    Number.isNaN(passengerCountNum) ||
    passengerCountNum < 1 ||
    passengerCountNum > 6
  ) {
...
async function submitForm() {
  ...
  try {
    ...
    const fallback =
      editingReservationId.value !== null ? 'Failed to update reservation.' : 'Failed to create reservation.';
